<template>
	<view>
		<uni-notice-bar class="notice" show-icon scrollable text="本网站/app/小程序永久免费,请勿通过任何付费渠道获得" :speed="200" />
		<uni-row class="demo-uni-row tip" width="730" style="font-size: 10px;">
			<uni-col :span="11">
				<view class="demo-uni-col dark">
					本站已运行
					<text style="color:red">{{ timeD }}</text>
				</view>
			</uni-col>
			<uni-col :span="11">
				<view class="demo-uni-col light">
					成功为
					<text style="color:red">{{ usersCount }}</text>
					位小可耐云跑步
				</view>
			</uni-col>
		</uni-row>
...
export default {
	...
	watch: {
		...
		query(newValue, oldValue) {
			if (!newValue) this.isShowList = true
			else {
	...
	methods: {
		...
		async getUsersList(callback) {
			this.isLoading = true
			const { data: res } = await this.$http.get('/api/sunrun', this.queryObject)
		...
		async getSerachList(e) {
			const { data: res } = await this.$http.get('/api/sunrun', {
				salvage: e
			})
			if (res.status !== 200) this.$showMsg()
		...
		getTimeD() {
			setInterval(() => (this.timeD = this.timeToDur(new Date().getTime(), 1665098803000, true)), 1000)
		}
	},
	onLoad() {
		this.getUsersList()
